...
#define  PHOTO_BMP          "./photo0.bmp"	//BMP格式图片存储路径
#define  PHOTO_YUV            "./photo0.yuv"	//YUV格式图片存储路径
...
int main(void)
{
	...
	//新建一个二进制文件YUV
	fp2 = fopen(PHOTO_YUV, "wb");
	if(!fp2)
	{
		printf("open "PHOTO_YUV" error\n");
		return FALSE;
	}
	...
	fwrite(buffers[0].start, PHOTO_WIDTH*PHOTO_HEIGHT*2, 1, fp2);
	printf("save "PHOTO_YUV" OK\n");

	set_bmp_header();
	yuyv_2_rgb888();
	//直接对bf、bi操作会出现bmp图片提示伪数据头而无法打开的现象
	//fwrite(&bf, 14, 1, fp1);
	//fwrite(&bi, 40, 1, fp1); 
	//简化主函数
	fwrite_bmp(fp1);
