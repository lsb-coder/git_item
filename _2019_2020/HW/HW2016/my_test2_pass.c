...
int main()
{
	...
	EL *errordlogs = (EL *)malloc(sizeof(EL));
	int n =0;
	char path[100];
	char f_name[20],*p;
	int line;
	
	//the reason of timeout is that the candition have not "!= EOF")
	while (scanf("%s %d",path,&line) != EOF) {
		...
		if (exist == 0) {
			if (n == 0) {
				...
				n++;
				errordlogs=(EL *)realloc(errordlogs, sizeof(EL)*n);
	...
	for (int i = 0;i < n && i < 8;i++) {
		...
		printf("%s %d %d\n",fn_final,errordlogs[i].line,errordlogs[i].num);
	}
